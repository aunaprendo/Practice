...
const updateResultsList = () => {
  ...
  units.forEach ((unit) =>{
		if (unit !== unitToConvert.value) {
			const conversion= processIngredient(ingredientQuantity.value, unitToConvert.value, unit, numberOfServings.value);
			resultList.innerHTML += `<li class="mb-3"><strong>${ingredientName.value}:</strong> ${conversion} ${unit}</li>
			`
		}
	})
}
